...
print("\n=== charts ===")
bar_chart(a1, [w1], [S1], f"TTC price position vs competitors — {w1}", PNG_W1)
bar_chart(a2, [w2], [S2], f"TTC price position vs competitors — {w2}", PNG_W2)
# Secondary: the two side by side, ordered by movement
bar_chart(top.sort_values("change_pts"), [w1, w2], [S1, S2],
          "TTC price position — both windows", PNG_BOTH)
...
def window_sheet(win):
    d = wide[wide.WINDOW == win].set_index("country")
